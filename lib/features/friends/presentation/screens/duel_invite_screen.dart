...
class DuelInviteScreen extends ConsumerWidget {
  ...
  void _decline(BuildContext context, WidgetRef ref) {
    ref
        ...
        .respondToInvite(inviteId: invite.id, accept: false);
    if (context.canPop()) {
      context.pop();
    } else {
  ...
  void _accept(BuildContext context, WidgetRef ref) {
    ref
        .read(duelControllerProvider.notifier)
        .respondToInvite(inviteId: invite.id, accept: true);
    context.pushReplacement(AppRoutes.duelGame);
  }

  static String _displayName(DuelInvite invite) {
    final String name = [
      invite.fromUser.firstName,
      invite.fromUser.lastName,
    ].where((part) => part != null && part.isNotEmpty).join(' ');
    if (name.isNotEmpty) return name;
    if (invite.fromUser.username != null &&
        invite.fromUser.username!.isNotEmpty) {
  ...
  static String _initials(DuelInvite invite) {
    final String first = (invite.fromUser.firstName?.isNotEmpty ?? false)
        ? invite.fromUser.firstName![0]
        : '';
    final String last = (invite.fromUser.lastName?.isNotEmpty ?? false)
        ? invite.fromUser.lastName![0]
        : '';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final AvatarColorOption avatarColor = AvatarColorOption.fromApiValue(
      invite.fromUser.avatarColor,
    );
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              FadeSlideIn(
                child: CloseHeader(
                  title: context.t.duelInvite.title,
                  onClose: () => _decline(context, ref),
                ),
              ),
              AppSpacing.xl.vGap,
              FadeSlideIn(
              ...
              FadeSlideIn(
                ...
                child: Center(child: DuelCategoryChip(category: category)),
              ),
              AppSpacing.xl.vGap,
              FadeSlideIn(
