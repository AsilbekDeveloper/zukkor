...
class LobbyResultScreen extends ConsumerWidget {
  ...
  List<LeaderboardEntry> _ranked(LobbyRoomState room) {
    final Map<String, LobbyParticipant> byId = {
      for (final p in room.participants) p.id: p,
    };
    return [
      for (int i = 0; i < args.result.standings.length; i++)
        if (byId[args.result.standings[i].participantId]
            case final LobbyParticipant participant)
          _entryFor(
            participant,
            args.result.standings[i],
            room.youParticipantId,
            i + 1,
          ),
    ];
  }

  LeaderboardEntry _entryFor(
    LobbyParticipant participant,
    LobbyPlayerScore score,
    String youId,
    int rank,
  ) {
    final LobbyPlayer player = LobbyPlayer.fromEntity(
      participant,
  ...
  // xonada yagona "yutdim/yutqazdim" degan narsa yo'q — shu sabab faqat
  // 1-o'rin "g'alaba" sifatida kattaroq his qildiriladi, qolgan har
  // qanday o'rin esa neytral, yengil haptic bilan kifoyalanadi
  // ([[duel_result_screen]]).
  void _rankHaptic(List<LeaderboardEntry> ranked) {
    final bool youAreFirst = ranked.any((e) => e.isCurrentUser && e.rank == 1);
    if (youAreFirst) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final List<LeaderboardEntry> podium = hasPodium
          ...
          child: Column(
            ...
            children: [
              ...
              FadeSlideIn(
                ...
                child: QuestionBreakdownList(items: args.result.breakdown),
              ),
              AppSpacing.xxl.vGap,
              if (isHost) ...[
                FadeSlideIn(
